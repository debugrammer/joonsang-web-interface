...
- 커밋 `66cac4c` → master 푸시 → `v2.0.2` 태그 푸시. v2.0.1 태그는 누락되어
  있었는데 소유자가 소급 생성 (d8f13ad — 태그 이력 v2.0.0~v2.0.2 정렬 확인).
- 소유자가 서버 배포 후 운영 검증(HTTP — HTTPS는 의도적 미구성): `/status`
  ALIVE, SPA 라우트 5종 200, 실존 자산 200, **누락 자산 404**(이번 릴리즈
  신규 서버 동작 = 배포 반영의 결정적 증거), `/foo.bar` SPA 폴백 200,
  index.html `no-cache` 헤더 정상.
- 관찰: `BUILD_VERSION`은 주입되지만 소비 코드가 없어 번들에서 트리셰이킹됨 —
  원격 배포 버전 확인 용도가 필요해지면 소비처(예: /status 응답) 추가 검토.
