  ...
  workExperienceData: [
    {
      ...
      content:
        '- 관제 서비스 구축<br />1. Graylog에 수집된 각 Cafe24 서비스들의 API 로그를 Graylog API를 이용하여 시각화 인터페이스 구축<br />2. 서비스를 API 시스템과 웹 인터페이스 시스템으로 분리하여 설계<br />3. 각 시스템은 L7 로드밸런서를 통해 분배되어 무중단 배포가 가능하도록 시스템 설계<br />4. 웹 인터페이스 시스템 - Nuxt.js SSR 모드에 Vuetify를 얹어 구현<br />3. API 시스템 - Java 11을 사용하여 Spring Boot로 구현<br /><br />- 관제 시스템 구축<br />1. 각 쇼핑몰 서버에서 Cafe24 API 로그를 중앙 로그 수집 시스템인 Graylog에 UDP GELF 형식으로 전송하여 수집하도록 설계<br />2. API 분석/연구에 용이하도록 로그 데이터 디자인<br />3. Graylog 웹 인터페이스에 Cafe24 API 위기관리 대시보드 디자인<br /><br />- 쇼핑몰 솔루션 프레임워크 유지보수<br />1. 몰프론트 서버에만 적용되어 있던 DB 리플리케이션을 어드민 서버에도 적용하기 위해 프레임워크 DB커넥션 핸들러와 모델 개선<br />2. 레거시 프레임워크에 Composer를 셋업하여 PSR-4 방식으로 오토로드 가능하도록 개선<br />3. JSON Request Body를 프레임워크 단에서 핸들링할 수 있도록 개선'
    },
    {
      start: '2017-04',
      end: '2017-06',
